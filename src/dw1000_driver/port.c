 ...
 * Copyright 2023 (c) Burak Kavak <burak@kavak.dev>
 *
 *
 * All rights reserved.
 *
...
static const struct gpio_dt_spec GPIO_DT = GPIO_DT_SPEC_GET(DT_NODELABEL(ieee802154), int_gpios);
...
{
    printk("%s: Binding to pin %d\n", __func__, GPIO_DT.pin);
    ...
    /* Decawave interrupt */
    gpio_pin_configure(GPIO_DT.port, GPIO_DT.pin, (GPIO_INPUT | GPIO_DT.dt_flags));

    gpio_init_callback(&gpio_cb, (gpio_callback_handler_t)(deca_isr), BIT(GPIO_DT.pin));
