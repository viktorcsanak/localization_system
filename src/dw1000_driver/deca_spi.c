...
#define SPI2_NODE_ID DT_NODELABEL(spi2)
...
int openspi(void)
{
    const struct gpio_dt_spec gpio_dev = GPIO_DT_SPEC_GET(SPI2_NODE_ID, cs_gpios);
    ...
    cs_ctrl = (struct spi_cs_control){
        .gpio = GPIO_DT_SPEC_GET(SPI2_NODE_ID, cs_gpios),
        .delay = 0U
    };
    ...
    spi = DEVICE_DT_GET(SPI2_NODE_ID);
    if (!spi) {
